...
This repository contains PyTorch and JAX implementations of the Lion optimizer discovered by symbnolic program search. Lion is available on [Praxis](https://github.com/google/praxis/blob/main/praxis/optimizers.py).
...
We only tune those hyperparameters in language tasks (Section~\ref{sec:lm}),
...
Since the effective weight decay is $lr * \lambda$, the value of $\lambda$ used for Lion is 10x larger than that for AdamW in order to maintain a similar strength.
For instance,
